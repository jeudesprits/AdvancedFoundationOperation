...
/**
 The subclass of `Foundation.Operation` from which all other operations should be derived.
...
class Operation: Foundation.Operation {
  ...
  private enum State: Int, Comparable {
    ...
    static fileprivate func <(lhs: State, rhs: State) -> Bool {
  ...
  private var state: State {
    get {
      return stateLock.withCriticalSection { _state }
    }
    set(newState) {
      /*
  ...
  var isUserInitiated: Bool {
    ...
    set(newQuality) {
      ...
      qualityOfService = newQuality ? .userInitiated : .default
    }
  }
   ...
   */
  func execute() {
    print("\(type(of: self).description()) must override `execute()`.")
  ...
  override final func waitUntilFinished() {
     ...
     */
    fatalError(
      "Waiting on operations is an anti-pattern. Remove this ONLY if you're absolutely sure there is No Other Way™."
    )
  }
}
